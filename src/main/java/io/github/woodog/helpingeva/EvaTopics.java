...
public class EvaTopics {
	...
	public void initTopics() {
		...
		final Map<String, Object> valuesToSerialize = 
		...
		for (Map.Entry<String, Object> entry : valuesToSerialize.entrySet()) {
			...
			plugin.getLogger().info("Key: " +
					plugin.getConfig().getConfigurationSection(topicKey).getValues(false));
	...
	public List<String> getHelpTopicText(String topic) {
		...
		if (topicType.equals("string")) {
			plugin.getLogger().info("helpTopicType: string");
			helpTopicText.add(
					plugin.colorString(((EvaHelpTopic) this.helpTopics.get(topic)).getTopicLine()));
		}
		
		if (topicType.equals("stringList")) {
			plugin.getLogger().info("helpTopicType: stringList");
